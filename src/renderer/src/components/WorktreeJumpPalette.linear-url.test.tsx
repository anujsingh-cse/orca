...
describe('WorktreeJumpPalette Linear URL intent', () => {
  ...
  it('selects the loading row and replaces it with a resolved issue preview', async () => {
    ...
    const loadingRow = testContainer.querySelector<HTMLElement>(
    ...
    expect(loadingRow?.dataset.cmdJLinearIssueState).toBe('loading')
    expect(loadingRow?.textContent).toContain('STA-4084')
    expect(loadingRow?.textContent).toContain('Loading Linear issue…')
  ...
  it('does not let a stale lookup replace a newer URL preview', async () => {
    ...
    await flushEffects()
    await act(async () => setCommandQuery?.('https://linear.app/stably/issue/STA-4099/second'))
    await flushEffects()
